...
const uploadKYCDocument = async (req) => {
  const documentPath = `/uploads/${req.file.filename}`;
  return await prisma.documents.create({
    data: {
        owner_type: "customer",
      owner_id: req.user.customer_id, // Link to the claim we just made
      document_type: "KYCDocument",
...
const getCustomerById = async (id) => {
  ...
  if (customer && customer.date_of_birth) {
    // Formatting it before returning it to the controller
