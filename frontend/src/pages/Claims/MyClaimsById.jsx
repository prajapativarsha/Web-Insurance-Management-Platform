...
const MyClaimsById = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await claimApi.getMyClaimsById(id);
        setClaims(res.data);
      } catch (err) {
  ...
  return (
    <div className="max-w-5xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Claim History for Policy #{id}</h2>
        <Link
          to={`/claims/my-claims/${id}/new-claim`}
      ...
      {claims.length === 0 ? (
        <p className="text-gray-600 border p-4 rounded bg-gray-50">No claims have been made for this policy yet.</p>
      ) : (
